...
class MonarchClassicTokensCollector {
    ...
    emit(startOffset, type) {
        ...
        let token = this._lastToken = new Token(startOffset, type, this._language)
    ...
    _myTokenize(line, lineState, offsetDelta, tokensCollector) {
        ...
        while (pos < lineLength) {
            ...
            if (Array.isArray(result)) {
                ...
                let emitted = tokensCollector.emit(pos0 + offsetDelta, tokenType);
                emitted.stack = stack
                if(stack != prevStack) {
                    // console.log('changed stack',MonarchStackElement.getStackElementId(stack),MonarchStackElement.getStackElementId(prevStack));
                    prevStack = stack
                    // emitted.stack = stack
                }

                if(stackTokens && false) {
                    let stackId = MonarchStackElement.getStackElementId(stack);
                    emitted.stack = stackId;
...
export {register,getLexer,tokenize,TokenTheme,parseTokenTheme}